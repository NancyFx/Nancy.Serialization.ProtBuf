...
assemblyinfo :version => [:clean] do |asm|
  ...
  asm.version = NANCY_VERSION
	...
	asm.product_name = "Nancy.Serialization.ProtoBuf"
	asm.title = "Nancy.Serialization.ProtoBuf"
	asm.description = "A ProtoBuf Serializer for the Nancy web framework"
...
zip :package => [:publish] do |zip|
    ...
    zip.output_file = "Nancy.Serialization.ProtoBuf-Latest.zip"
...
task :nuget_package => [:publish] do
    ...
    nuspecs = FileList["src/nancy.serialization.protobuf/*.nuspec"]
    root = File.dirname(__FILE__)

    # Copy all project *.nuspec to nuget build folder before editing
    FileUtils.cp_r nuspecs, "#{OUTPUT}/nuget"
    nuspecs = FileList["#{OUTPUT}/nuget/nancy.serialization.protobuf.nuspec"]
    ...
    nuspecs.each do |nuspec|
        update_xml nuspec do |xml|
            ...
            xml.root.elements["metadata/licenseUrl"].text = "https://github.com/NancyFx/Nancy.Serialization.ProtBuf/blob/master/license.txt"
