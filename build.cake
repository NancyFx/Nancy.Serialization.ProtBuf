  ...
  .Does(() =>
{
  ...
  else
  {
    ...
    using(var process = StartAndReturnProcess("mono", new ProcessSettings{Arguments =
      xunit + " " +
      "test/Nancy.Serialization.ProtoBuf.Tests/bin/"+configuration+"/net452/unix-x64/Nancy.Serialization.ProtoBuf.Tests.dll"}))
    {
      process.WaitForExit();
...
Task("Package")
  ...
  .Does(() =>
{
  var package = outputPackages + File("Nancy.Serialization.ProtoBuf-Latest.zip");
